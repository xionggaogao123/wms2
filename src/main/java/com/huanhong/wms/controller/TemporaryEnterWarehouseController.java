...
public class TemporaryEnterWarehouseController extends BaseController {
    ...
    @Resource
    private TemporaryEnterWarehouseV1Service temporaryEnterWarehouseV1Service;
    ...
    public Result add(@PathVariable("id") Long id) {
        log.info("临时入库查询id为:{}",id);
        try {
            return temporaryEnterWarehouseV1Service.findById(id);
        } catch (Exception e) {
            return Result.failure(9999,e.getMessage(),"查询失败");
        }
    }
    ...
    @PutMapping("/update")
    public Result update(@Valid @RequestBody UpdateTemporaryEnterWarehouseRequest updateTemporaryEnterWarehouseRequest) {
        log.info("临时入库更新主表子表数据为:{}", JsonUtil.obj2String(updateTemporaryEnterWarehouseRequest));
        try {
            return temporaryEnterWarehouseV1Service.updateTemporaryEnterWarehouse(updateTemporaryEnterWarehouseRequest);
        } catch (Exception e) {
            return Result.failure(Constant.STATUS_9999,e.getMessage(),"更新临库入库单失败");
        }
    }
    ...
    public Result delete(@PathVariable("id") Long id) {
        try {
            return temporaryEnterWarehouseV1Service.deleteById(id);
        } catch (Exception e) {
            return Result.failure(Constant.STATUS_9999,e.getMessage(),"删除临时入库数据失败");
        }
    }
