...
public class BalanceLibraryDetailServiceImpl extends SuperServiceImpl<BalanceLibraryDetailMapper, BalanceLibraryDetail> implements IBalanceLibraryDetailService {
    ...
    @Override
    public Result createProcurementPlan(BalanceLibraryDetail balanceLibraryDetail) {
        Integer balanceLibraryDetailId = balanceLibraryDetail.getId();
        if (null == balanceLibraryDetailId) {
            return Result.failure(400,"id不可为空");
        }
        BalanceLibraryDetail detail = this.baseMapper.selectById(balanceLibraryDetailId);
        if (null == detail) {
        ...
        //待采购数量
        double purchasedQuantity = NumberUtil.sub(detail.getApprovedQuantity().doubleValue(),sumCalibrationQuantity);
        // 创建采购计划
